...
def gen_user_data(num_user_records: int) -> None:
    for id in range(num_user_records):
        ...
        # update username and product name 10% of the time
...
# Generate a random user agent string (for readability)
def random_user_agent():
    return fake.user_agent()

# Generate a random IP address (for readability)
def random_ip():
    return fake.ipv4()
...
def generate_click_event(user_id, product_id=None):
    ...
    # return as dictionary for convenience when uploading data
    click_event = {
        "click_id": click_id,
        "user_id": user_id,
